...
class modFreelancehuntReviewsHelper
{
	/**
	 * Get items layout
	 *
	...
	public static function getAjax()
	{
		if ($params = self::getModuleParams(Factory::getApplication()->input->get('module_id', 0)))
		{
			...
			{
				ob_start();
				require ModuleHelper::getLayoutPath('mod_freelancehunt_reviews', $params->get('layout', 'default') . '_items');
	...
	{
		// Send request
		$url  = 'https://api.freelancehunt.com/profiles/me?include=reviews';
		$curl = curl_init();
		...
		// Prepare items
		$items = array();
		foreach ($profile->get('reviews', array()) as $i => $item)
		{
			if ($limit == 0 || (($i >= $offset && $i < ($offset + $limit))))
			{
				$from = new Registry($item->from);
				$from->set('referral', $profile->get('referral'));

				$project = new Registry($item->project);
				$project->set('referral', $profile->get('referral'));
